...
В данном репозитории представлены обязательные для скачивания моды: automodpack, ferritcore и modernfix. И остальные моды - клиентские на оптимизацию, которые рекомендуется поставить. Все основные моды будут автоматически докачены во время захода на сервер.

Для того чтобы скачать репозиторий, нужно нажать на зеленую кнопку Code, затем Download Zip.
...
|:----------|:------------|:---------------|
| **ОЗУ** | 12 ГБ (выделить 6 ГБ) | 16 ГБ (выделить 8 ГБ) |
...
-->
## Установка
0. Если у вас не установлена **[Java](https://www.java.com/ru/)**, установите. Нажмите выгрузить Java для пк.
1. Скачайте и установите **[NeoForge 1.21.1](https://neoforged.net/)** (необходим vpn для захода на сайт) или же скачайте из этого репозитория файл neoforge-21.1.226-installer.
  ...
  * Если скачиваете из репозитория, то следующие 2 шага можно пропустить.
  * Перейдите на [официальный сайт NeoForge](https://neoforged.net/)
  * Выберите версию **1.21.1** и скачайте установщик
  * Запустите `.jar` файл (или же открыть с помощью Java)
  * Выберите **«Install client»** (установка клиента)
  * Укажите путь к папке `.minecraft` (обычно выбран автоматически)
  * Нажмите **«OK»** и дождитесь завершения установки
  </details>

2. Скачайте все моды они находятся в папке recomended_client_modpack (моды - файлы `.jar`)
3. Поместите `.jar` файлы в папку `mods` (`.minecraft/mods`). Легче всего ее найти, если зайти в майнкрафт, затем нажать кнопку моды и папка с модами. 2 способ: зайти в проводник и написать `%appdata%/.minecraft` и выбрать папку mods.
...
- Замените `2` на `6` или `8` (например: `-Xmx6G`). Можно и `4`, но мы не можем гарантировать стабильной игры.
- Нажмите **«Сохранить»**

**CurseForge Launcher: (писал дипсик, не проверено)**
